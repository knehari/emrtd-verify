 ...
 * Design v2 : voile en fondu, feuille qui monte du bas (`sheetUp`, 0,42 s, courbe iOS), feuille
 * #1C1C1E et carte #2C2C2E en sombre.
 */
...
export function ShareModal({ demo }: { demo: AuthentikDemo }) {
  ...
  useEffect(() => {
    if (!demo.showShare) return;
    rise.setValue(0);
    Animated.timing(rise, { toValue: 1, duration: 420, easing: Easing.bezier(0.32, 0.72, 0, 1), useNativeDriver: true }).start();
  }, [demo.showShare, rise]);
  const translateY = rise.interpolate({ inputRange: [0, 1], outputRange: [height / 2, 0] });
  return (
...
const makeStyles = (colors: PaletteColors) => StyleSheet.create({
  ...
  sheet: { backgroundColor: colors.sheet, borderTopLeftRadius: 14, borderTopRightRadius: 14, paddingTop: 10, paddingHorizontal: 12, paddingBottom: 34 },
  handle: { width: 36, height: 5, borderRadius: 3, backgroundColor: `rgba(${colors.inkBaseRgb},0.25)`, alignSelf: "center", marginBottom: 14 },
  card: { backgroundColor: colors.sheetCard, borderRadius: 14, padding: 16, marginHorizontal: 4, marginBottom: 12 },
  ...
  cancelBtn: { flex: 1, borderRadius: 13, backgroundColor: colors.sheetCard, paddingVertical: 15, alignItems: "center" },
