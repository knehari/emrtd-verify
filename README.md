...
Ce qui est **réellement implémenté et testé** (207 tests automatisés, TypeScript + Python) :
...
- Backend des interfaces d'administration et du portail tenant multi-tenant : authentification admin/tenant (cookie JWT httpOnly, RBAC), CRUD clients KYC et comptes administrateurs, revue globale et par tenant des vérifications, suivi consolidé des "clients" par tenant (vérifié/non vérifié/en attente/à surveiller) avec isolation multi-tenant stricte vérifiée par des tests d'intégration réels (`apps/api/src/modules/{admin-auth,admin-api,tenant-auth,tenant-api,verified-person}` — voir [docs/admin-web.md](docs/admin-web.md) et [docs/tenant-portal.md](docs/tenant-portal.md)).
- Interface d'administration interne (`apps/admin-web`, Next.js 14) : connexion, tableau de bord (KPIs + répartition des verdicts), CRUD tenants avec révélation de clé API une seule fois, revue globale des vérifications, gestion des comptes admin (RBAC). Vérifiée par un build de production réel et un parcours utilisateur complet en navigateur (voir [docs/admin-web.md](docs/admin-web.md) "Vérification").
- Portail tenant (`apps/tenant-portal`, Next.js 14, palette visuelle distincte de `apps/admin-web`) : connexion, tableau de bord, suivi consolidé des "clients" (vérifié/non vérifié/en attente/à surveiller) avec changement de statut, revue des vérifications du tenant, profil. Isolation multi-tenant stricte côté serveur (jamais un paramètre client). Vérifié par un build de production réel et un parcours utilisateur complet en navigateur (voir [docs/tenant-portal.md](docs/tenant-portal.md) "Vérification").
- Prometheus + Grafana provisionnés dans `docker-compose.yml`, dashboard versionné en JSON branché sur les `/metrics` réels de l'API et du worker, lien depuis `apps/admin-web` (voir [docs/admin-web.md](docs/admin-web.md) "Dashboard Grafana").
