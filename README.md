...
- Portail tenant (`apps/tenant-portal`, Next.js 14, palette visuelle distincte de `apps/admin-web`) : connexion, tableau de bord, suivi consolidé des "clients" (vérifié/non vérifié/en attente/à surveiller) avec changement de statut, revue des vérifications du tenant, profil. Isolation multi-tenant stricte côté serveur (jamais un paramètre client). Vérifié par un build de production réel et un parcours utilisateur complet en navigateur (voir [docs/tenant-portal.md](docs/tenant-portal.md) "Vérification"). Le dashboard Grafana reste à construire.
