...
- Interface d'administration interne (`apps/admin-web`, Next.js 14) : connexion, tableau de bord (KPIs + répartition des verdicts), CRUD tenants avec révélation de clé API une seule fois, revue globale des vérifications, gestion des comptes admin (RBAC). Vérifiée par un build de production réel et un parcours utilisateur complet en navigateur (voir [docs/admin-web.md](docs/admin-web.md) "Vérification"). Le portail tenant (`apps/tenant-portal`) et le dashboard Grafana restent à construire.
