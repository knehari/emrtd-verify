...
- [x] Synchronisation avec l'ICAO PKD — récupération réelle (HTTPS ou LDAP, `packages/pki-trust/src/pkdClient.ts`), décodage/vérification cryptographique de la CSCA Master List (`packages/pki-trust/src/{masterListAsn1,masterList}.ts`, CMS SignedData + ancres de confiance du Master List Signer épinglées hors bande), et synchronisation backend avec bascule atomique et persistance (`apps/api/src/modules/pki/csca-sync.service.ts`, job quotidien `CscaSyncScheduler`). Testé de bout en bout (`packages/pki-trust/test/{masterList,pkdClient}.test.ts`, `apps/api/test/pki/csca-sync.service.test.ts`) avec une Master List synthétique réellement signée, y compris les cas de rejet (aucune ancre configurée, source indisponible) où le pointeur de confiance actif n'est jamais modifié. **Non vérifié** : l'accès réel à l'annuaire LDAP officiel ICAO PKD, qui nécessite un enregistrement/des identifiants que ce projet n'a pas — voir [pki-trust-model.md](pki-trust-model.md#synchronisation-de-la-csca-master-list) pour le détail de ce qui reste non testable en conditions réelles.
