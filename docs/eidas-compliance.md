...
|---|---|---|
| Vérification de documents d'identité, y compris lecture NFC eMRTD, avec voies "Automated"/"Manual"/"Hybrid Validation" | Toute la crypto en aval (Passive Authentication, Active/Chip Authentication, chaîne CSCA) est implémentée et testée. Le protocole de lecture NFC BAC lui-même (APDU, messagerie sécurisée) est implémenté, **validé byte-exact contre l'exemple travaillé officiel ICAO Doc 9303 Part 11 Appendix D.2/D.3/D.4**, et entièrement portable React Native (`packages/emrtd-core/src/nfc/`, branché dans `apps/mobile/src/nfc/emrtdReader.ts`) — voir [pvid-compliance.md](pvid-compliance.md) pour le détail | **Bloquant, mais réduit à un seul point.** La conformité à la spécification est désormais confirmée byte-exact ; seul manque un test contre un document/lecteur NFC réel. PACE (ECDH-GM/CAM, repli BAC) implémenté et validé byte-exact contre Doc 9303 Part 11 Appendix G.1 |
| Détection de vivacité / résistance aux attaques de présentation (photo, vidéo, masque, deepfake) — référence implicite ISO/IEC 30107 | `check_liveness()` (`services/face-match/app/liveness.py`) reste **passive** (résolution/netteté/nombre de visages). Un **protocole de liveness active** (challenge-réponse à séquence d'actions aléatoire, conçu pour une capture ARKit TrueDepth — voir `docs/facial-recognition.md` "Détection de vivacité active") est désormais implémenté et testé côté serveur (`packages/emrtd-core/src/liveness/`, `apps/api`), mais **la capture native (ARKit) elle-même n'a volontairement pas été écrite** (même raison que la lecture NFC : impossible à vérifier sans Xcode/appareil physique/compte Apple Developer payant) | **Réduit à un point, comme la lecture NFC.** Le protocole (génération/vérification du challenge, résistance au rejeu/à l'injection) est conforme et testé ; seul manque le module natif de capture. Aucun test PAD formel (ISO/IEC 30107-3) mené |
...
| Contre-mesures aux attaques par instrument (photo, replay, masque, deepfake, morphing) | Partiellement — la chaîne cryptographique protège contre le clonage/l'altération de la puce ; un protocole de liveness active résistant au rejeu photo/vidéo/deepfake pré-rendu est implémenté et testé côté serveur, mais sans capture native branchée en production (voir section 3). Ne couvre pas un deepfake piloté en temps réel par un opérateur humain (menace ENISA distincte) |
| Approche combinée "best of breed"/"mix-and-match" selon le risque | Le design par client KYC (seuils configurables, politique de risque par client) va dans ce sens, et le protocole de liveness active est prêt côté serveur ; reste à compléter par la lecture NFC et la capture native ARKit pour être une combinaison réellement multi-facteurs bout en bout |
...
1. **Validation en conditions réelles de la lecture NFC BAC** — bloquant pour
   ...
   NFC. PACE (ECDH-GM/CAM) est implémenté et validé byte-exact contre Doc 9303
   Part 11 Appendix G.1.
2. **Détection de vivacité active** — protocole de challenge-réponse implémenté
   et testé côté serveur (`packages/emrtd-core/src/liveness/`, `apps/api`, voir
   `docs/facial-recognition.md`), conçu pour une capture ARKit TrueDepth ; il
   manque encore le module natif de capture lui-même (même blocage que la
   lecture NFC — voir `apps/mobile/src/liveness/faceLivenessSession.ts`).
   Condition pour ETSI 119 461, LoA "high", PVID.
